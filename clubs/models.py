...
class Meeting(models.Model):
    ...
    club = models.ForeignKey(Club, related_name="meeting_club", on_delete=models.PROTECT)
    URL = models.CharField(max_length=520, blank=False)
    member_selected = models.ManyToManyField(User, symmetrical=True, related_name="meeting_members")
    ...
    def get_member(self):
        return "\n".join([p.username for p in self.member_selected.all()])

    def add_meeting(self, meeting):
        if meeting not in self.meeting.all():
            meeting.meeting_members.add(self)

    def get_club_member(self):
        print(self.club.meeting_club.members.all())
