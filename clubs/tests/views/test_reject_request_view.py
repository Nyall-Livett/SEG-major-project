...
class FollowToggleTest(TestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse('reject_request', kwargs={'user_id': self.followee.id})

    def test_accept_request_url(self):
        self.assertEqual(self.url,f'/reject_request/{self.followee.id}')
    ...
    def test_get_accept_request_for_followee(self):
        ...
        self.user.reject_request(self.followee)
        response = self.client.get(self.url, follow=True)
        user_follow_requests_after_reject = self.user.follow_requests.count()
        user_followers_count_after_reject = self.user.followers_count()
        self.assertEqual(user_follow_requests_after, user_follow_requests_after_reject+1)
        self.assertEqual(user_followers_count_after, user_followers_count_after_reject)
    ...
    def test_get_follow_toggle_with_invalid_id(self):
        self.client.login(username=self.user.username, password='Password123')
        url = reverse('reject_request', kwargs={'user_id': self.followee.id+100000})
