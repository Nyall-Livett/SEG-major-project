...
class MarkAllActedUponNotificationsTestCase(TestCase, LogInTester):
    ...
    def setUp(self):
        self.url = reverse('notification_mark_all_acted_upon')
        self.default_user = User.objects.get(pk = 1)
        self.default_notification = Notification.objects.get(pk = 1)

    def test_url(self):
        self.assertEqual(self.url,'/notification_mark_all_acted_upon/')
    ...
    def test_notification_acted_upon_to_true(self):
        self.client.login(username=self.default_user, password='Password123')
        self.assertTrue(self._is_logged_in())
    ...
    def test_multiple_notifications_gets_acted_upon_at_once_true(self):
        self.client.login(username=self.default_user, password='Password123')
        self.assertTrue(self._is_logged_in())
