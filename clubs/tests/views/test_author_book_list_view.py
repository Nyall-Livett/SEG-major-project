...
class AuthorBookListTest(TestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse('get_books_by_author', kwargs={'book_id': self.book.id})

    def test_book_list_url(self):
        self.assertEqual(self.url,f'/get_books_by_author/{self.book.id}')
    ...
    def test_get_author_book_list(self):
        ...
        self.assertTemplateUsed(response, 'books_by_author.html')
        ...
        for book_id in range(settings.BOOKS_PER_PAGE-1):
            self.assertContains(response, f'isbnauthor{book_id}')
    ...
    def test_get_author_book_list_with_pagination(self):
        self.client.login(username=self.user.username, password='Password123')
        ...
        page_one_url = reverse('get_books_by_author', kwargs={'book_id': self.book.id}) + '?page=1'
        ...
        self.assertFalse(page_obj.has_previous())
        self.assertTrue(page_obj.has_next())
        page_two_url = reverse('get_books_by_author', kwargs={'book_id': self.book.id}) + '?page=2'
        ...
        self.assertTrue(page_obj.has_next())
        page_three_url = reverse('get_books_by_author', kwargs={'book_id': self.book.id}) + '?page=3'
    ...
    def test_get_author_book_list_redirects_when_not_logged_in(self):
